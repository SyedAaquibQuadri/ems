  ...
  return (
    <div className='flex min-h-screen w-screen items-center justify-center bg-[#0f0f0f] px-4 py-8'>
      <div className='bg-[#181818] border border-[#2a2a2a] rounded-2xl p-7 w-full max-w-sm'>
        ...
        onClick={() => {
          if (!mode) {
            setToast({ type: 'error', msg: 'Please select Create or Join first.' })
            return
          }
          if (mode === 'join' && !orgSlug.trim()) {
            setToast({ type: 'error', msg: 'Please enter your organization code first.' })
            return
          }

          const stateData = { role: mode === 'create' ? 'admin' : 'employee' }
          if (mode === 'join') stateData.companyCode = orgSlug.trim()
